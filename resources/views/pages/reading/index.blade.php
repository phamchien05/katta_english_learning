<x-app-layout :title="$title" :icon="$icon">
    @php
        // Màu nền theo chủ đề (mục 6) - viết literal để Tailwind JIT quét thấy
        $topicStyle = [
            'technology' => ['bg' => 'bg-amber-50 border-amber-100', 'icon' => 'bg-amber-100 text-amber-600', 'lucide' => 'cpu'],
            'science' => ['bg' => 'bg-sky-50 border-sky-100', 'icon' => 'bg-sky-100 text-sky-600', 'lucide' => 'flask-conical'],
            'environment' => ['bg' => 'bg-emerald-50 border-emerald-100', 'icon' => 'bg-emerald-100 text-emerald-600', 'lucide' => 'leaf'],
            'history' => ['bg' => 'bg-orange-50 border-orange-100', 'icon' => 'bg-orange-100 text-orange-600', 'lucide' => 'landmark'],
            'society' => ['bg' => 'bg-purple-50 border-purple-100', 'icon' => 'bg-purple-100 text-purple-600', 'lucide' => 'users'],
            'business' => ['bg' => 'bg-blue-50 border-blue-100', 'icon' => 'bg-blue-100 text-blue-600', 'lucide' => 'briefcase'],
            'health' => ['bg' => 'bg-rose-50 border-rose-100', 'icon' => 'bg-rose-100 text-katta-accent', 'lucide' => 'heart-pulse'],
            'general' => ['bg' => 'bg-white border-gray-100', 'icon' => 'bg-gray-100 text-gray-500', 'lucide' => 'book-open'],
        ];
    @endphp
    ...
    <div class="max-w-6xl mx-auto mt-6" x-data="{ tab: 'all', topicFilter: 'all', q: '' }">
        ...
        <div class="flex items-center gap-2 mb-5">
            ...
            <button type="button" @click="tab = 'history'"
                ...
                {{ __('reading.tab_history') }} ({{ $history->count() }})
            </button>
        </div>

        {{-- ===== Tab: Tất cả (danh sách nhóm theo chủ đề) ===== --}}
        <div x-show="tab === 'all'">
            <h1 class="text-2xl font-bold text-gray-800">{{ __('nav.reading') }}</h1>
            <p class="text-gray-400 mt-1">{{ __('reading.subtitle') }}</p>
            ...
            <div class="relative mt-4 max-w-md">
                ...
                <input type="text" x-model="q" placeholder="{{ __('reading.search_placeholder') }}"
            ...
            <div class="flex flex-wrap gap-2 mt-4">
                ...
                @foreach ($topics as $topic)
                    <button type="button" @click="topicFilter = '{{ $topic }}'"
                        ...
                        {{ __('reading.topics.' . $topic) }}
                    </button>
                @endforeach
            </div>

            {{-- Danh sách theo từng chủ đề --}}
                ...
                @forelse ($grouped as $topic => $passages)
                    ...
                    <div x-show="topicFilter === 'all' || topicFilter === '{{ $topic }}'">
                        <div class="flex items-center gap-2 mb-3">
                            ...
                            <h2 class="font-semibold text-gray-700">{{ __('reading.topics.' . $topic) }}</h2>
                        </div>

                        @if ($topic === 'general')
                            {{-- Tổng quan: tổ chức theo cấp độ CEFR, bấm vào random 1 bài trong kho của cấp đó
                                 (giống Từ vựng/Dịch) - không phải bấm thẳng vào 1 bài cố định --}}
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                @foreach (['A1', 'A2', 'B1', 'B2', 'C1'] as $level)
                                    <a href="{{ route('reading.general', $level) }}"
                                       class="rounded-2xl border p-4 hover:shadow-md hover:-translate-y-0.5 transition {{ $style['bg'] }}">
                                        ...
                                        <p class="font-semibold text-gray-800 text-sm leading-snug">{{ $level }} Reading</p>
                                        <p class="text-xs text-gray-400 mt-0.5">{{ $level }}</p>
                                        <p class="text-xs text-katta-primary font-semibold mt-3">{{ __('reading.start_reading') }}</p>
                                    </a>
                                @endforeach
                            </div>
                        @else
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                @foreach ($passages as $passage)
                                    <a href="{{ route('reading.show', $passage) }}"
                        ...
                        <div class="bg-white rounded-2xl shadow-sm p-4 flex items-center gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-xs text-gray-400 mb-0.5">
                                    {{ __('reading.topics.' . ($item->passage->topic ?? 'general')) }} · {{ $item->created_at->diffForHumans() }}
                                </p>
                                <p class="text-sm text-gray-700 font-semibold truncate">{{ $item->passage->title ?? '' }}</p>
                            </div>
                            @if ($item->passage)
                                <a href="{{ route('reading.show', $item->passage) }}"
